...
`;
  ...
  it("keeps the extension boundary sticky disk on one protected key", () => {
    ...
    expect(hostedLintCache.if).toBe(
      "matrix.task == 'lint' && steps.extension-boundary-inputs.outputs.enabled == 'true' && (vars.OPENCLAW_CI_RUNNER_BACKEND == 'github' || vars.OPENCLAW_CI_RUNNER_BACKEND == 'hybrid' || (github.event_name == 'pull_request' && github.event.pull_request.head.repo.full_name != github.repository))",
    );
